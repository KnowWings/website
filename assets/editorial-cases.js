(() => {
  ...
  let index = [1, 2, 3].includes(requestedCase) ? requestedCase : 1;
  let busy = false, start = null, dragged = false, timer;

  function logicalIndex(i = index) {
    if (i === 0) return originals.length - 1;
  ...
  function paint(animate = true) {
    ...
    cards.forEach((card, i) => {
      ...
      const title = card.querySelector('h3')?.textContent || 'case study';
    ...
    counter.textContent = `0${logicalIndex() + 1} / 0${originals.length}`;
    if (!animate) {
  ...
  function finish() {
    ...
    if (index !== oldIndex) {
      ...
      if (focused >= 0) {
        ...
        cards[focused + offset]?.querySelector('a')?.focus({preventScroll: true});
      }
    }
    busy = false;
  }
